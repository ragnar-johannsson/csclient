...
CloudStackClient.prototype.isoDate = function() {
	var d = new Date();
...
CloudStackClient.prototype.calculateSignature = function (queryDict) {
    ...
    var orderedQuery = qs.stringify(queryDict, { encode: true }).replace(/\%5B(\D*?)\%5D/g, '.$1').replace(/\%5B(\d*?)\%5D/g, '[$1]').split('&').sort().join('&').toLowerCase();
