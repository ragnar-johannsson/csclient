...
CloudStackClient.prototype.execute = function (cmd, params, callback) {
    ...
    apiURL.path += querystring.stringify(params).replace('+', '%20').replace('details%5B0%5D', 'details\[0\]');
...
CloudStackClient.prototype.calculateSignature = function (queryDict) {
    ...
    var orderedQuery = querystring.stringify(queryDict).replace('details%5B0%5D', 'details\[0\]').split('&').sort().join('&').toLowerCase();
